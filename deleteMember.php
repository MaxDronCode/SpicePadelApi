<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $q = "DELETE FROM member WHERE dni_m = '$dni'";
        $result = mysqli_query($conn, $q);

        if ($result) {
            header('Content-Type: application/json');
            echo json_encode(['success' => true, 'message' => 'Delete Member Ok']);
        } else {
            header('Content-Type: application/json');
            http_response_code(500); // Envía un código de estado HTTP cuando hay un error
            echo json_encode(['success' => false, 'message' => 'Delte Member Failed: ' . mysqli_error($conn)]);
        }
    }
?>
